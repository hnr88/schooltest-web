...
export const accessModelSchema = z.enum(['single_admin', 'managed_admins']);
 ...
 */
export const OWNERSHIP_TRANSFER_CODES = {
...
export type OwnershipTransferCode =
  (typeof OWNERSHIP_TRANSFER_CODES)[keyof typeof OWNERSHIP_TRANSFER_CODES];

/** POST /api/ops/schools/{documentId}/owner — the body (strict). */
export const ownershipTransferBodySchema = z.strictObject({
  owner_documentId: documentIdSchema,
  /** Optimistic guard: the owner the caller saw. null means "expect no owner". */
  expected_owner_documentId: documentIdSchema.nullable(),
});
export type OwnershipTransferBody = z.infer<typeof ownershipTransferBodySchema>;

/** 200 body. */
export const ownershipTransferResultSchema = z.strictObject({
  school_documentId: documentIdSchema,
  owner_documentId: documentIdSchema,
  previous_owner_documentId: documentIdSchema.nullable(),
});
export type OwnershipTransferResult = z.infer<typeof ownershipTransferResultSchema>;

export const ownershipTransferResponseSchema = dataEnvelope(ownershipTransferResultSchema);
...
export const OwnershipTransferOperation: OpsOperation<
  typeof ownershipTransferBodySchema,
  typeof ownershipTransferResponseSchema
> = Object.freeze({
  contractId: 'C-OPS-PORTAL-027',
  method: 'POST',
  path: '/api/ops/schools/{documentId}/owner',
  request: ownershipTransferBodySchema,
  response: ownershipTransferResponseSchema,
  success: 200,
