...
export const PORTAL_PLAN_TO_LEGACY_PLAN: Readonly<Record<PortalPlan, SchoolPlan>> = Object.freeze({
  pilot: 'trial',
  standard: 'full_license',
  enterprise: 'full_license',
});
 ...
 */
export const LEGACY_PLAN_TO_PORTAL_PLAN: Readonly<Record<SchoolPlan, PortalPlan>> = Object.freeze({
  trial: 'pilot',
  full_license: 'standard',
});

/** Server-side derivation rule a backfill or a read path calls directly. */
export function derivePortalPlan(legacyPlan: SchoolPlan, storedPortalPlan?: PortalPlan | null): PortalPlan {
  if (storedPortalPlan) return storedPortalPlan;
  return LEGACY_PLAN_TO_PORTAL_PLAN[legacyPlan];
}
