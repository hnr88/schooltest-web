...
// German showcase render (cookie-driven locale) — split from design-system.spec.ts (file cap).
const de = loadMessages('de');

test('DS German: NEXT_LOCALE=de renders the showcase from de.json', async ({
  browser,
  baseURL,
}) => {
  ...
  try {
    ...
    for (const key of ['sectionBrand', 'sectionCards', 'sectionData'] as const) {
      await expect(
        page.getByRole('heading', { level: 2, name: ds(de, key), exact: true }),
      ).toBeVisible();
    }
