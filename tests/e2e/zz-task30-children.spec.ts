...
test.describe('task 30: children v2 round-trip vs live C-CHD-01..04', () => {
  test('add with email/L1/ACARA -> roster -> edit -> archive frees the seat', async ({
    ...
    const screen = page.locator('[data-slot="school-children"]');
    await expect(screen).toBeVisible({ timeout: 20_000 });

    // ADD (C-CHD-02 v2): name, email, year level, Korean, emerging ACARA phase.
    await screen.getByRole('button', { name: cat(en, 'SchoolChildren.addButton'), exact: true }).click();
    await page.waitForURL('**/dashboard/school/children/new');
    const form = page.locator('[data-slot="school-child-new"]');
    await form.getByLabel(/Given name/).fill(givenName);
    await form.getByLabel(cat(en, 'SchoolChildren.form.familyName'), { exact: true }).fill(familyName);
    await form.getByLabel(cat(en, 'SchoolChildren.form.email'), { exact: true }).fill(email);
    await form.getByLabel(cat(en, 'SchoolChildren.form.yearLevel'), { exact: true }).selectOption('8');
    await form.getByLabel(cat(en, 'SchoolChildren.form.firstLanguage'), { exact: true }).selectOption('korean');
    // The ACARA control renders for the school_admin role only.
    const acaraSelect = form.getByLabel(cat(en, 'SchoolChildren.form.acaraPhase'), { exact: true });
    await expect(acaraSelect).toBeVisible();
    await acaraSelect.selectOption('emerging');
    await form
      .getByRole('button', { name: cat(en, 'SchoolChildren.form.submitCreate'), exact: true })
      .click();
    await page.waitForURL('**/dashboard/school/children', { timeout: 20_000 });
    ...
    await screen.getByLabel(cat(en, 'SchoolChildren.filters.searchLabel')).fill(familyName);
    const row = screen.getByRole('row', { name: new RegExp(familyName) });
    await expect(row).toBeVisible({ timeout: 10_000 });
    await expect(row.getByText(cat(en, 'SchoolChildren.table.statusActive'), { exact: true })).toBeVisible();
    ...
    const editItem = page.getByRole('menuitem', {
      name: cat(en, 'SchoolChildren.actions.edit'),
      exact: true,
    });
    ...
    await editDialog
      .getByLabel(cat(en, 'SchoolChildren.form.firstLanguage'), { exact: true })
      .selectOption('vietnamese');
    await editDialog
      .getByLabel(cat(en, 'SchoolChildren.form.acaraPhase'), { exact: true })
    ...
    await expect(
      page.getByText(cat(en, 'SchoolChildren.form.updatedToast').replace('{name}', fullName), {
        exact: true,
      }),
    ).toBeVisible();
    ...
    const archiveItem = page.getByRole('menuitem', {
      name: cat(en, 'SchoolChildren.actions.archive'),
      exact: true,
    });
    ...
    await confirm
      .getByRole('button', { name: cat(en, 'SchoolChildren.archiveDialog.confirm'), exact: true })
      .click();
    await expect(confirm).toBeHidden();
    await expect(
      row.getByText(cat(en, 'SchoolChildren.table.statusArchived'), { exact: true }),
    ).toBeVisible();
