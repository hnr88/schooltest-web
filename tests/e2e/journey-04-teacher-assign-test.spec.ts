...
import { choice, isLiveTab, isSessionWrite, modal, modalTab, waitForModalData } from './helpers/teacher-start-session-modal';
...
// as the journey teacher (t2), open "Start new session" on Live sessions, tick every
// free student (Select all), give them the first test and Start now (C-TS-1's real POST), land on the class's Live tab
...
test('assign a test to a class, open the sitting, and it all persists on reload', async ({ page, request }) => {
  ...
  // ASSIGN: the one Start-new-session modal, every free student (Select all), the first test, Start now.
  ...
  // The ASSIGNED STUDENTS: the monitor lists exactly who sits it — the students
  // Select all ticked, named explicitly (a legacy null row is the whole roster).
