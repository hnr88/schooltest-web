...
// loosened shape. Calls ride 429s out via fetchWithRetry (the per-IP window is
// shared by every lane on this stack); the strict status asserts still apply.
...
export async function createSession(
  ...
  const response = await fetchWithRetry(() =>
    request.post(`${API_BASE}/api/teacher/test-sessions`, {
      headers: { Authorization: `Bearer ${jwt}` },
...
export async function closeSession(
  ...
  const response = await fetchWithRetry(() =>
    request.post(`${API_BASE}/api/teacher/test-sessions/${sittingDocumentId}/close`, {
      headers: { Authorization: `Bearer ${jwt}` },
    }),
  );
