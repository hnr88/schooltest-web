...
import { cat, ds, escapeRegExp, heroTitleLines, home, icu, loadMessages, type Locale, type Messages } from './helpers/i18n';
...
const DS_SECTION_KEYS = ['sectionBrand', 'sectionButtons', 'sectionBadges', 'sectionAlerts', 'sectionCards', 'sectionForms', 'sectionOverlays', 'sectionData', 'sectionFeedback'] as const;
...
test('DS-VARIANTS: every showcase export renders with all variants', async ({ page }) => {
  ...
  for (const key of DS_SECTION_KEYS) await expect(page.getByRole('heading', { level: 2, name: ds(en, key), exact: true })).toBeVisible();
  ...
  const variants: [number, string, string][] = [
    [0, 'buttonCreate', 'button.ds-probe'], [0, 'buttonGetStarted', 'button.bg-navy-900'], // default, navy
    [0, 'buttonCreate', 'button.bg-accent'], [0, 'buttonSave', 'button.bg-secondary'],
    [0, 'buttonCancel', 'button.bg-background'], [0, 'buttonCancel', 'button:not(.bg-background)'], // outline, ghost
    [0, 'buttonDelete', 'button.bg-destructive'], [0, 'buttonLearnMore', 'button.text-primary'], // destructive, link
    [1, 'buttonGetStarted', 'button.bg-white'], [1, 'buttonLearnMore', 'button[class*="border-white/40"]'], // white, outline-white
  ];
  ...
  await expect(states.getByRole('button', { name: ds(en, 'buttonSave'), exact: true })).toBeDisabled();
  await expect(states.getByRole('button', { name: ds(en, 'buttonNotifications'), exact: true })).toBeVisible();

  // Badges & tags — 9 badge variants + status badges + tag + count badge.
  const badgeRows = page.locator('#badges > div');
  for (const [key, count] of [['badgeLive', 3], ['badgeScheduled', 3], ['badgeDraft', 2], ['tagGrade', 2]] as const) await expect(badgeRows.nth(0).getByText(ds(en, key), { exact: true })).toHaveCount(count);
  for (const cls of ['bg-navy-900', 'bg-teal-100', 'bg-green-100', 'bg-amber-100', 'bg-red-100']) await expect(badgeRows.nth(0).locator(`> span.${cls}`), cls).toHaveCount(1);
  await expect(badgeRows.nth(1).locator('[data-slot="status-badge"]')).toHaveCount(3);
  for (const key of ['badgeLive', 'badgeScheduled', 'badgeDraft'] as const) await expect(badgeRows.nth(1).getByText(ds(en, key), { exact: true })).toBeVisible();
  ...
  for (const key of ['alertInfoTitle', 'alertSuccessTitle', 'alertWarningTitle', 'alertErrorTitle'] as const) await expect(alerts.getByText(ds(en, key), { exact: true })).toBeVisible();
  const cards = page.locator('#cards');
  for (const key of ['statTests', 'statStudents', 'statScore', 'emptyTitle'] as const) await expect(cards.getByText(ds(en, key), { exact: true })).toBeVisible();
  await expect(cards.getByText(ds(en, 'featureDemoTitle'), { exact: true })).toHaveCount(2);
  await expect(cards.getByRole('button', { name: ds(en, 'emptyAction'), exact: true })).toBeVisible();
  await expect(page.locator('#ds-email')).toHaveAttribute('placeholder', ds(en, 'fieldEmailPlaceholder'));
  ...
  await expect(page.locator('#ds-search')).toHaveAttribute('placeholder', ds(en, 'fieldSearchPlaceholder'));
  await expect(page.locator('#ds-select')).toBeVisible();
  await expect(page.locator('#ds-native-select')).toBeVisible();
  for (const role of ['checkbox', 'radio', 'switch'] as const) await expect(page.getByRole(role)).toHaveCount(2);

  // Data display: tabs, segmented control, table + status pills, pagination, breadcrumb.
  const data = page.locator('#data');
  for (const key of ['tabsOverview', 'tabsQuestions', 'tabsResults'] as const) await expect(data.getByRole('tab', { name: ds(en, key), exact: true })).toBeVisible();
  const segmented = data.locator('[data-slot="segmented-control"]');
  for (const key of ['segmentedWeek', 'segmentedMonth', 'segmentedYear'] as const) await expect(segmented.getByRole('button', { name: ds(en, key), exact: true })).toBeVisible();
  await expect(data.locator('[data-slot="table-caption"]')).toHaveText(ds(en, 'tableCaption'));
  for (const key of ['tableRowMath', 'tableRowScience', 'tableRowHistory', 'tableRowReading'] as const) await expect(data.getByText(ds(en, key), { exact: true })).toBeVisible();
  ...
  for (const [alt, count] of [['logoLockup', 1], ['logoMark', 1], ['logoWhite', 2]] as const) await expect(brand.getByRole('img', { name: ds(en, alt), exact: true })).toHaveCount(count);
...
test('LOCALE-TOGGLE: footer switcher en→de→en updates content without reload', async ({ page }) => {
  ...
  await footer.getByRole('combobox', { name: cat(en, 'LocaleSwitcher.label'), exact: true }).click();
  ...
  expect(await page.evaluate(() => (window as unknown as Record<string, unknown>).__noReload)).toBe(1);
  await footer.getByRole('combobox', { name: cat(de, 'LocaleSwitcher.label'), exact: true }).click();
