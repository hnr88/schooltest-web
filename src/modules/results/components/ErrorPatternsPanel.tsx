...
export function dominantPattern(view: ResultView): { type: string; pct: number } | null {  const dominant = view.error_patterns.reduce<{ type: string; pct: number } | null>(
...
export function ErrorPatternsPanel({ view, studentName }: { view: ResultView; studentName: string }) {
  ...
  return (
    <section data-slot="error-patterns" aria-label={t('errorPatternsHeading', { name: first })} className="flex flex-col gap-2">
      <h2 className="text-caption font-bold uppercase tracking-wide text-muted-foreground">
        {t('errorPatternsHeading', { name: first })}
      </h2>
      <ul className="flex flex-col gap-2.5">
        {view.error_patterns.map((pattern) => {
          const known = PATTERN_KEY[pattern.type];
          return (
            <li key={pattern.type} data-slot="error-pattern" data-type={pattern.type} data-pct={pattern.pct} className="flex flex-col gap-1">
              ...
              <span className="text-caption text-muted-foreground">
                {known !== undefined ? t(`errorPattern.${known}.note`) : t('errorPatternFallbackNote')}
              </span>
            </li>
          );
        })}
      ...
      {dominant !== null ? (
        <p data-slot="error-pattern-insight" data-type={dominant.type} className="text-caption font-semibold">
          {t('errorPatternInsight', {
            label: PATTERN_KEY[dominant.type] !== undefined ? t(`errorPattern.${PATTERN_KEY[dominant.type]}.label`) : dominant.type,
            name: first,
            pct: dominant.pct,
          })}
        </p>
