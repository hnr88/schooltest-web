...
// 13.5/500 navy, subtle #F1F5F9 (muted) hover — the spec never uses teal for
// menu highlights, so the vendored primitives were re-pointed from bg-accent
// to bg-muted at the source.
export const CONTENT_CLASSES =
  'w-auto min-w-[212px] rounded-panel border border-[#EEF1F6] shadow-[0_16px_40px_rgba(14,35,80,0.18)] ring-0 p-1.5';

export const ITEM_CLASSES =
  'gap-2.5 rounded-[10px] px-3 py-2.5 text-[13.5px] font-medium focus:bg-muted focus:text-foreground not-data-[variant=destructive]:focus:**:text-foreground [&_svg]:text-slate-500';
