...
function MiniStatTile({ value, label, tone = 'default', className }: MiniStatTileProps) {
  return (
    ...
    >
      <span className={cn('truncate text-stat-sm font-bold', VALUE_TONES[tone])} title={value}>
        {value}
      </span>
      {/* slate-600, not muted-foreground: 12.5px on the #F7F9FC tile needs 4.5:1 */}
      <span className="break-words text-meta text-balance text-slate-600">{label}</span>
    </div>
  );
}
