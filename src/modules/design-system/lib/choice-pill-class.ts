...
export function pillClass(size: ChoicePillSize, selected: boolean, disabled?: boolean) {
  return cn(
    'relative inline-flex min-w-0 max-w-full items-center rounded-full border transition-colors duration-200 ease-out-expo after:absolute after:inset-x-0 focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none motion-reduce:transition-none',
    SIZE[size],
    selected
