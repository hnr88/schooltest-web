...
// the add/edit form schema, which mirrors the C-CHD-02 body exactly: name,
// date of birth, year level, the flat EAL/D background fields and an optional
// class assignment. Guardian, media and parent fields do not exist here by
// design — the server rejects them with a 400.
...
// The add/edit form (C-CHD-02 create / C-CHD-03 edit). Text controls stay
// strings on the form; numeric/boolean coercion happens at the boundary.
export function createSchoolChildFormSchema(tv: (key: string) => string) {
  return z.object({
    ...
    first_language: z.string().trim().max(100, tv('tooLong')),
