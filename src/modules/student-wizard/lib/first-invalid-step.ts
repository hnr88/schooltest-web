...
// fields. Returns null when nothing matches. Field groups follow the ACTIVE
// step list (task 47) so the returned index lands on a rendered step.
export function firstInvalidStep(errors: FieldErrors<StudentWizardValues>): number | null {
  const invalid = Object.keys(errors);
  if (invalid.length === 0) {
    return null;
  }
  const index = activeStepFields().findIndex((fields) =>
