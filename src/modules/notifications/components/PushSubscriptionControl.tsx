  ...
  return (
    ...
    >
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <div className="flex flex-wrap items-center gap-2">
            <h3 id="notification-push-title" className="text-sm font-semibold text-foreground">
              {t('notificationPreferences.push.title')}
            </h3>
            <Badge variant={config.variant}>{t(config.labelKey)}</Badge>
          </div>
          <p id="notification-push-description" className="text-sm text-muted-foreground">
      ...
      <Button
        ...
        className="min-h-11 shrink-0 px-4 disabled:bg-muted disabled:text-foreground disabled:opacity-100"
