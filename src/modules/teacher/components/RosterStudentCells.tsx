...
// The five data cells of one roster row (task 33, now the kit's column cells —
// ops/34). Every value is printed, never re-derived: the score is
// `overall.domain_score` ("No result yet" where the student has no official
// Result — never a 0; a present result with no score renders the em dash),
// `delta_display` verbatim, the weakest DISPLAY skill from the pure layer
// (not-assessed is a gap, never weak; Critical — no posterior, no band — never
// compares), the ACARA phase as a chip and the ⚠ confidence marker. The
// data-slots each cell carried inside the old row are unchanged.
//
...
export function RosterScoreCell({ row }: RosterStudentCellsProps) {
...
export function RosterWeakestCell({ row }: RosterStudentCellsProps) {
  const t = useTranslations('Teacher.results.students');
  const view = row.result;
...
export function RosterAcaraCell({ row }: RosterStudentCellsProps) {
...
export function RosterConfidenceCell({ row }: RosterStudentCellsProps) {
  const t = useTranslations('Teacher.results.students');
  const view = row.result;
