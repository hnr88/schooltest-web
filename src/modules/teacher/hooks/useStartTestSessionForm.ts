...
import { useTeacherClassesQuery } from '@/modules/teacher/queries/use-teacher-classes.query';
 ...
 * view 1). Both pickers are fed by live reads — the teacher's own classes
 * (C-8 `GET /api/classes`, ownership forced server-side) and the selectable
 * A/B pair (C-TD-2 `GET /api/teacher/tests`) — and the submit posts C-TS-1.
 *
...
export function useStartTestSessionForm() {
  ...
  const classes = useTeacherClassesQuery();
  ...
  const classOptions = toClassOptions(classes.data ?? []);
  ...
  return {
    ...
    status: deriveSetupStatus({
      isLoading: classes.isLoading || tests.isLoading,
      isError: classes.isError || tests.isError,
      isSuccess: classes.isSuccess && tests.isSuccess,
